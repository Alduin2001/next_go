...
export const RegisterComp:FC = ()=>{
    ...
    return(
        <form className="w-4/5" onSubmit={handleSubmit(submitForm)}>
            <div className="my-2">
            <Input {...register('name')} placeholder="Введите имя" className="outline-none border-b-2 border-green-500 focus:shadow-md w-full"/>
            {errors.name?.message && (<p className="text-red-500 italic mt-2">*{errors.name.message}</p>)}
            </div>
            <div className="my-2">
            <Input {...register('surname')} placeholder="Введите фамилия" className="outline-none border-b-2 border-green-500 focus:shadow-md w-full"/>
            {errors.surname?.message && (<p className="text-red-500 italic mt-2">*{errors.surname.message}</p>)}
            </div>
            <div className="my-2">
            ...
            <Row className="mt-2">
                <Col col={6}>
                    <Button type="submit" className="rounded-md cursor-pointer w-full p-2 bg-green-500 text-[#eee]">Зарегистрироваться</Button>
                </Col>
                <Col col={6}>
                    <Button className="rounded-md w-full p-2 bg-red-500 text-[#eee]">Очистить</Button>
                </Col>
            </Row>
            <p className="mt-2">У вас уже есть аккаунт? <Link href="/auth/login" className="text-blue-500">Войти</Link></p>
        </form>
    )
}